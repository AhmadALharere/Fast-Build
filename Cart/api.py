...
def is_collection_valid(request):
        ...
        if not compitability_MotherBoard_Memory(part_sorter['MotherBoard'],part_sorter['Memory']):
            return JsonResponse({'build_status':'Success','cart_status ':'Danger','massege':'the MotherBoard can`t be compitable with all selected Memorys'})

        if not compitability_Case_MotherBoard(part_sorter['Case'],part_sorter['MotherBoard']):
            return JsonResponse({'build_status':'Success','cart_status ':'Danger','massege':'the Case cannot contain MotherBoard'})

        if not compitability_Case_PowerSupply(part_sorter['Case'],part_sorter['PowerSupply']):
            return JsonResponse({'build_status':'Success','cart_status ':'Danger','massege':'the Case cannot contain PowerSupply or it has already one i it'})

        if not compitability_MotherBoard_CPUCooler(part_sorter['MotherBoard'],part_sorter['CPUCooler']):
            return JsonResponse({'build_status':'Success','cart_status ':'Danger','massege':'the MotherBoard is not supported Lequid Cooler'})

        if not compitability_Case_VideoCard(part_sorter['Case'],part_sorter['VideoCard']):
            return JsonResponse({'build_status':'Success','cart_status ':'Danger','massege':'the Case maybe cannot contain GPU Card'})

        if not compitability_Case_CPUCooler(part_sorter['Case'],part_sorter['CPUCooler']):
            return JsonResponse({'build_status':'Success','cart_status ':'Danger','massege':'the Case maybe cannot contain CPU Cooler'})
        ...
        if not compitability_MotherBoard_Extention_Cards(part_sorter['MotherBoard'],part_sorter['SoundCard'],part_sorter['VideoCard'],part_sorter['WiresNetworkCard'],part_sorter['WirelessNetworkCard']):
            return JsonResponse({'build_status':'Success','cart_status ':'Warning','massege':'the MotherBoard don`t have enough or suitable sluts to import all Extintial Cards'})
            
        if not compitability_MotherBoard_InternalHardDrives(part_sorter['MotherBoard'],part_sorter['InternalHardDrive']):
            return JsonResponse({'build_status':'Success','cart_status ':'Warning','massege':'the MotherBoard don`t have enough or suitable sluts to import all Hard Drives'})

        if not compitability_Case_InternalHardDrive(part_sorter['Case'],part_sorter['InternalHardDrive']):
            return JsonResponse({'build_status':'Success','cart_status ':'Warning','massege':'the Case maybe cannot contain all Hard-Drives'})

        if not compitability_Case_OpticalDrive(part_sorter['Case'],part_sorter['OpticalDrive']):
            return JsonResponse({'build_status':'Success','cart_status ':'Warning','massege':'the Case maybe cannot contain all OpticalDrive'})

        if not compitability_Case_CaseFan(part_sorter['Case'],part_sorter['CaseFan']):
            return JsonResponse({'build_status':'Success','cart_status ':'Warning','massege':'the Case maybe cannot contain all Fans'})

        if not compitability_CaseFan(part_sorter['CaseFan'],part_sorter['MotherBoard'],part_sorter['FanController']):
            return JsonResponse({'build_status':'Success','cart_status ':'Warning','massege':'there is no enough channels to connect all Fans'})

        if not compitability_CPU_Memory(part_sorter['CPU'],part_sorter['Memory']):
            return JsonResponse({'build_status':'Success','cart_status ':'Warning','massege':'The CPU cannot deal with all this memory'})

        if not compitability_CPU_Cooling(part_sorter['CPU']):
            return JsonResponse({'build_status':'Success','cart_status ':'Warning','massege':'The CPU has already cooler with it'})


        return JsonResponse({'build_status':'Success','cart_status ':'Success','massege':'The part that you had selected must be fit togather'})
